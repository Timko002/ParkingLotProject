...
int LotF::reserve(time_t startTime, time_t endTime)
{
	tm* st = new tm();
...
int LotF::getNoOfTotallyBookedSpots()
{
...
string LotF::getFirstAvailableSlot(int currHour, int currMin)
{
	int starting_index;
	...
	for (int i = 0; i < pSpaceF.size(); i++)
	{
		available_block = pSpaceF[i]->getFirstAvailableSlot(starting_index);
